...
if (isset($_POST['update_status'])) {
    ...
    $order_query = mysqli_query($conn, "SELECT product_id FROM cart WHERE id = '$order_id'");
    $order_row = mysqli_fetch_assoc($order_query);
    $product_id = $order_row['product_id'];

    mysqli_query($conn, "DELETE FROM `cart` WHERE id = '$order_id'") or die('Deletion query failed');
    exit(); 
}
}
?>
